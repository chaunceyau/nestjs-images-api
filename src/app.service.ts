...
export class AppService {
  ...
  getSignedImageAccessUrl(fileKey: string, options: {
  ...
  getSizeConditionString(requestedFileSizeBytes: number, maxFileSizeBytes?: number) {
  ...
  async generateSignedUploadUrl(options: GenerateSignedUploadUrl) {
    const Conditions: any[] = [{ acl: "public-read" }, { bucket: "test-signed-requests" }];
...
interface GenerateSignedUploadUrl {
  maxSizeBytes?: number
  fileName: string
  fileId: string
  type: string
